...
import 'package:brokerBook/controller/QueryController.dart';
import 'package:brokerBook/model/SendMessageResponse.dart';
...
import '../ui/DashboardModule/DashboardScreenUI.dart';
...
class EstateCardList extends StatelessWidget {
      ...
      itemBuilder: (context, index) {
        ...
        return MultiSelectItem(
            ...
            onSelected: () {
              ...
              if (!estateList[index].isMyProperty!) {
                AppCommonFunction.flutterToast("Premium are not selectable", false);
              } else {
  ...
  Widget ListCardContainer(
      {required IconData icon, required String text, double fontSize = 16, double iconSize = 17, Color iconColor = AppColors.primaryColor}) {
    return Wrap(
      children: [
        Icon(
          icon,
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 10),
                        estateList[index].area!.isEmpty ? Container() : ListCardContainer(icon: Icons.location_on, text: estateList[index].area!),
                        SizedBox(height: 7),
                        ...
                        estateList[index].numberOfBedrooms!.toString() == "0"
                            ...
                            : ListCardContainer(
                                ...
                                text: estateList[index].numberOfBedrooms.toString(),
                              ),
                ...
                estateList[index].isMyProperty!
                  ? Container()
                  : WidgetButton(text: "Contact Broker", fontsize: 18,height: 36, onPressed: () {
                  QueryController queryController = QueryController();
                  queryController.sendMessageApi(
                    token: PreferenceHelper().getUserData().authToken!,
                    phNumber: "broker_book_assistant",
                    sms:  false,
                    whatsApp: true,
                    estateId: [estateList[index].id!],
                  ).then((response) {
                    if(response != null){
